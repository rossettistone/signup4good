...
function create_organization() {
  return {
    org_name:'Foo Org', email:'foo@bar.org'
    contact_name:'Bob Organizer', address:'555 Foo Lane',
    event_ids:[]
  }
}
...
Meteor.startup(function () {
  ...
  if (Organization.find().count() === 0) {
    ...
    for (var i = 0; i < org_names.length; i++) {
      ...
      Organization.insert(default_org, function (err, org_id) {
        ...
        for(var i = 0; i < event_names; i++) {
          ...
          new_org = Organization.findOne({_id:org_id})
          new_org.event_ids.push(new_event._id)
